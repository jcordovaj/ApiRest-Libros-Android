...
class BookRepository(private val apiService: BooksApiService) {
    ...
    /**
     * Llama a la API y procesa la respuesta para añadir la lógica de negocio fake (disponibilidad).
     * @param query La cadena de búsqueda.
     */
    suspend fun searchBooksAndProcess(query: String): List<BookDisplay> {
        val response = apiService.searchBooks(query)

        // Mapea la respuesta de la API a la vista con la lógica de negocio agregada
        return response.items?.map { bookItem ->
            ...
            val stock       = if (isAvailable) Random.nextInt(1,
                5) else 0 // Si hay disponibilidad, retorna stock entre 1-4 libros
